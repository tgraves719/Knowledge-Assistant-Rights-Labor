...
def _build_platform_query_prompt(request: "QueryRequest", chunks: list[dict], conversation_context: str = "") -> str:
    ...
    return (
        ...
        "When the correct answer depends on the worker's job classification (for example wages, "
        "wage steps, seniority, scheduling, or rules that differ by role) and their classification "
        "has not been established in the conversation, ask a brief, friendly question to get it "
        "before answering — for example 'What's your job classification?' — rather than guessing or "
        "answering for every role at once. If they have already told you their classification, reuse "
        "it and do not ask again. If the answer does not depend on classification, just answer.\n"
